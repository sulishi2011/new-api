...
func buildChannelListQuery(group string, statusFilter int, typeFilter int, archived bool) *gorm.DB {
	query := model.DB.Model(&model.Channel{}).Where("archived = ?", archived)
...
func GetAllChannels(c *gin.Context) {
	...
	if enableTagMode {
		tags, err := model.GetPaginatedChannelTags(buildChannelListQuery(groupFilter, statusFilter, typeFilter, archivedFilter), pageInfo.GetStartIdx(), pageInfo.GetPageSize())
		if err != nil {
			common.SysError("failed to get paginated tags: " + err.Error())
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "获取标签失败，请稍后重试"})
			return
		}
		total, err = model.CountChannelTags(buildChannelListQuery(groupFilter, statusFilter, typeFilter, archivedFilter))
		if err != nil {
		...
		for _, tag := range tags {
			...
			query := buildChannelListQuery(groupFilter, statusFilter, typeFilter, archivedFilter)
			if *tag == "" {
				query = query.Where("tag IS NULL OR tag = ?", "")
			} else {
		...
		if err := buildChannelListQuery(groupFilter, statusFilter, typeFilter, archivedFilter).Count(&total).Error; err != nil {
			common.SysError("failed to count channels: " + err.Error())
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "获取渠道数量失败，请稍后重试"})
			return
		}

		err := sortOptions.Apply(buildChannelListQuery(groupFilter, statusFilter, typeFilter, archivedFilter)).
			Preload("VendorProfile").
	...
	countQuery := buildChannelListQuery(groupFilter, statusFilter, -1, false)
	var results []struct {
		Type  int64
		Count int64
	}
	...
	archiveCountQuery := buildChannelListQuery(groupFilter, statusFilter, -1, true)
...
func SearchChannels(c *gin.Context) {
	...
	if enableTagMode {
		...
		for _, tag := range tags {
			...
			query := buildChannelListQuery(group, -1, -1, archivedFilter)
			if *tag == "" {
				query = query.Where("tag IS NULL OR tag = ?", "")
			} else {
				query = query.Where("tag = ?", *tag)
		...
		channels, err := model.SearchChannelsWithArchive(keyword, group, modelKeyword, idSort, archivedFilter, sortOptions)
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": err.Error(),
